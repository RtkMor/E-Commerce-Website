...
const SearchProducts = () => {
    const { query } = useParams(); // Use useParams to get the search query
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const response = await fetch(ApiSummary.searchProduct.url, {
                    method: ApiSummary.searchProduct.method,
                ...
                const sortedProducts = sortProducts(result.data || []);
    ...
    return (
        <div className="container-sm mx-auto my-4 px-2 md:px-4">
            <div className="flex justify-between ms-2 me-2 px-8 mb-3 text-center md:text-left">
                <h1 className="text-xl md:text-2xl font-semibold capitalize">Search results for: {query}</h1>
                <select
                    name="sort"
